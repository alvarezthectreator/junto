import React, { useState, useEffect, useRef } from 'react';
  ...
  function readStoredCurrentUserSnapshot() {
    ...
    try {
      const storedUser = sessionStorage.getItem('junto-current-user');
                ...
                <div className="relative px-6 pb-6 sm:pb-8">
                  <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 -mt-16 sm:-mt-20 mb-6">
                    <div className="relative inline-block">
                      <div className={`h-28 w-28 sm:h-36 sm:w-36 overflow-hidden rounded-2xl border-[4px] shadow-2xl ${isLightMode ? 'border-[#FAF8F5] bg-white' : 'border-[#0B0B0E] bg-[#141419]'}`}>
                        <img
                  ...
                  <div className="max-w-2xl">
                    ...
                    <p className={`mt-4 text-sm leading-relaxed ${isLightMode ? 'text-amber-900/80' : 'text-gray-300'}`}>
                      {profile.bio || `Hey! I'm ${profile.name}. Let's connect.`}
                    </p>
                  </div>
                </div>
              </div>
                      ...
                      {profile.interests.map((interest) => (
                        <span
                          ...
                          className={`rounded-xl border px-3 py-1.5 text-xs font-medium ${
                            isLightMode ? 'bg-amber-50 text-amber-950 border-amber-900/10' : 'bg-white/[0.02] text-gray-300 border-white/[0.05]'
                          }`}
                        >
