...
export const Nearby: React.FC<NearbyProps> = ({
  ...
  const handleRequestVibe = async (vibeId: string) => {
    if (!requestedVibes.includes(vibeId)) {
      setRequestedVibes([...requestedVibes, vibeId]);
      
      // Call API to request to join the vibe
      try {
        if (currentUser?.id) {
          await API.swipeUser(currentUser.id, vibeId, 'request');
        }
      } catch (error) {
                ...
                <div style={{ padding: 18 }}>
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                      <button
                        onClick={() => handleRequestVibe(selectedVibe.id)}
                        style={{
                          border: "none",
                          borderRadius: 999,
                          background: requestedVibes.includes(selectedVibe.id) ? "#10B981" : "#F59E0B",
                          color: "#111",
                          padding: "10px 16px",
                          fontSize: 13,
                          fontWeight: 700,
                      ...
                      >
                        {requestedVibes.includes(selectedVibe.id) ? "✓ Requested" : "Request"}
                      </button>
                      <button
